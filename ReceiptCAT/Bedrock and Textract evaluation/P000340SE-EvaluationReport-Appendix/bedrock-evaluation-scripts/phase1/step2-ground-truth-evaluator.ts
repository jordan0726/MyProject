...
interface Row {
  ...
  claude37_output: string;
  claude4_output: string;
  mixtral_output: string;
  mistral7b_output: string;
}
...
function printCategoryAccuracyTable(rows: Row[], models: string[], categories: string[]) {
  ...
  for (const row of rows) {
    const truth = row.ground_truth_category?.trim();
...
function main() {
  const inputPath = path.resolve("data/bedrock_eval/phase1/step2_ground_truth_eval/bedrock_phase1_ground_truth_eval.csv");
  const csvData = fs.readFileSync(inputPath, "utf8");
  const rows: Row[] = Papa.parse<Row>(csvData, { header: true }).data;

  const models = ["claude37_output", "claude4_output", "mixtral_output", "mistral7b_output"];
  const categories = Array.from(new Set(rows.map(r => r.ground_truth_category).filter(Boolean)));
  ...
  for (const row of rows) {
    ...
    for (const model of models) {
      ...
      if (pred === truth) {
        ...
        // false negative for truth
        metrics[model][truth].fn++;
        // false positive for predicted category (if valid)
